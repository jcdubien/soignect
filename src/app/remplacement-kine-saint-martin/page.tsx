...
export const metadata: Metadata = {
  title: "Remplacement kiné Saint-Martin — Annonces & disponibilités | ParaBoard",
  description:
  ...
  openGraph: {
    title: "Remplacement kiné Saint-Martin | ParaBoard",
    description: "Le job board des kinésithérapeutes de Saint-Martin.",
    type: "website",
  },
};

const SAINT_MARTIN_COMMUNES = ["Marigot (Saint-Martin)", "Grand Case (Saint-Martin)"];

async function getMissions() {
  return prisma.mission.findMany({
    where: {
      isActive: true,
      location: { in: SAINT_MARTIN_COMMUNES },
    },
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-base leading-relaxed">
          ParaBoard connecte les kinésithérapeutes de Saint-Martin — collectivité d&apos;outre-mer distincte
      ...
      <h2 className="text-lg font-bold text-gray-800 mb-4">
        {missions.length > 0
          ? `${missions.length} annonce${missions.length > 1 ? "s" : ""} à Saint-Martin`
      ...
      <div className="mt-12 border-t border-gray-100 pt-8 space-y-3 text-sm text-gray-400">
        <h2 className="text-base font-semibold text-gray-600">Remplacement kiné à Saint-Martin</h2>
        <p>
          Saint-Martin est une collectivité d&apos;outre-mer française distincte de la Guadeloupe.
          Son marché de la kinésithérapie est spécifique : patientèle internationale, tourisme médical,
          et une communauté de soignants restreinte. Les remplacements y sont souvent bien rémunérés.
        </p>
        <div className="flex flex-wrap gap-2 pt-2">
          <Link href="/remplacement-kine-guadeloupe" className="underline hover:text-kine-600">Guadeloupe</Link>
          <span>·</span>
          <Link href="/remplacement-kine-saint-barth" className="underline hover:text-kine-600">Saint-Barth</Link>
        </div>
      </div>
    </div>
  );
