...
// CONTRAT SALARIÉ ENTRE INFIRMIERS À DURÉE INDÉTERMINÉE (section 260).
//
...
//
// SOURCE : « Modèle de contrat salarié entre infirmiers à durée indéterminée à remplir » (CNOI,
// version du 03/06/2025). 21 articles — un de moins que le CDD, qui porte en plus l'indemnité de
// précarité ; rupture et transmission à l'Ordre y sont donc numérotées 20 et 21, contre 21 et 22.
//
// ── POURQUOI CE FICHIER DUPLIQUE LARGEMENT LE CDD, ET POURQUOI C'EST VOULU ───────────────────
//
// Les deux modèles partagent environ 90 % de leur texte, et ce dépôt factorise d'ordinaire bien
// moins que ça. La règle est ici SUSPENDUE, sur un raisonnement précis :
//
// Le diff des deux sources montre que des articles d'apparence identique diffèrent en réalité —
// l'article 9 du CDI ajoute « (carte grise, assurance du véhicule …) », l'article 14 exige un
// préavis de congés « au moins deux mois à l'avance » là où le CDD dit « dans un délai
// raisonnable », l'article 12 reformule. Partager ce texte imposerait de paramétrer chacun de ces
// écarts, et surtout : une retouche future au texte « commun » modifierait EN SILENCE deux
// transcriptions de deux documents officiels distincts, qui peuvent évoluer séparément.
//
...
export function buildInfirmierSalariatCdiPdf(d: ContractDataSalarieInfirmier) {
  const {
    ...
    vehicule, preavisMois, lieuSignature, generatedAt,
  ...
  return (
    <Document>
      <Page size="A4" style={S.page}>
        ...
        <View style={S.header}>
          <Text style={S.title}>Contrat salarié entre infirmiers à durée indéterminée</Text>
        ...
        <View style={S.article}>
          ...
          <Text style={S.body}>
            {periodeEssaiMois !== null ? (
              <>
                Le présent contrat ne deviendra définitif qu'à l'expiration d'une période d'essai
                fixée à {periodeEssaiMois} mois.
                {"\n\n"}
                Dans la mesure où cette période d'essai constitue une période de travail effectif,
                toute absence du salarié, quel qu'en soit le motif, aura pour conséquence de
                prolonger la période d'essai d'une durée équivalente à l'absence.
                {"\n\n"}
                Au cours de la période d'essai, chacune des parties peut rompre le contrat à
                condition de respecter le délai de prévenance prévu aux articles L.1221-25 et
                L.1221-26 du Code du travail.
                {"\n\n"}
                Cette période d'essai pourra faire l'objet d'un renouvellement dans les conditions
                prévues à l'article L.1221-21 du Code du travail.
              </>
            ) : (
              <>
        ...
        <View style={S.article}>
          ...
          {vehicule.type === "PERSONNEL" ? (
            ...
            <Text style={[S.body, { marginTop: 6 }]}>
              ...
              {"\n\n"}
              L'employeur met à disposition du salarié les papiers du véhicule (carte grise, assurance
        ...
        <View style={S.article}>
          ...
          <Text style={S.body}>
            ...
            informations et renseignements dont il pourra avoir connaissance de par l'exercice de ses
            fonctions. Il s'interdit donc de divulguer à qui que ce soit ces renseignements ou
            informations.
            {"\n\n"}
        ...
        <View style={S.article}>
          ...
          <Text style={S.body}>
            ...
            {"\n\n"}
            La date de ses congés est déterminée par accord entre l'employeur et lui-même au moins deux
            mois à l'avance, de telle façon que la continuité des soins soit assurée.
            {"\n\n"}
        ...
        <View style={S.article}>
          <Text style={S.articleTitle}>Article 20 — Rupture</Text>
          <Text style={S.body}>
          ...
          <Text style={[S.articleTitle, { fontSize: 9.5, marginTop: 6 }]}>20.1 — Rupture du contrat pour cause de décès de l'employeur</Text>
          <Text style={S.body}>
        ...
        <View style={S.article}>
          <Text style={S.articleTitle}>Article 21 — Transmission à l'Ordre</Text>
          <Text style={S.body}>
        ...
        {/* Bloc de clôture présent dans le modèle CDI, absent du CDD. Le troisième exemplaire
            est celui du Conseil départemental : le modèle le dit, on ne l'abrège pas. */}
        <View style={S.article}>
          <Text style={S.body}>
            Fait à <Val v={lieuSignature} fallback="[lieu à compléter]" />, le{" "}
            {fmtDateUTC(generatedAt)}.
            {"\n"}
        ...
        <Text style={S.footer} fixed>
          ...
          de l'Ordre des infirmiers (03/06/2025) · Généré par Soignect
        </Text>
      </Page>
    </Document>
  );
